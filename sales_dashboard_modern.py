...
st.sidebar.image("https://i.imgur.com/5RHR6Ku.png", use_container_width=True)
...
with tab_overview:
    ...
    monthly = filtered.set_index('OrderDate').resample('ME').sum(numeric_only=True)['Sales'].reset_index()
    ...
    fig4 = px.bar(topprods, x="Sales", y="ProductName", orientation='h', color="Sales",
                  color_continuous_scale="viridis", template="plotly_white", title="Top 10 Products by Sales")
    st.plotly_chart(fig4, use_container_width=True)

# --- Forecast & Decomposition Tab ---
with tab_forecast:
    st.markdown("### Forecasting & Seasonality Analysis")
    monthly_sales = filtered.set_index('OrderDate').resample('ME').sum(numeric_only=True)['Sales']
...
with tab_anomalies:
    ...
    monthly_sales = filtered.set_index('OrderDate').resample('ME').sum(numeric_only=True)['Sales'].reset_index()
